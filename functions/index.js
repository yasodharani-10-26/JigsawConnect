 ...
 */
exports.createHackerRankTeam = functions.https.onCall(async (data, context) => {
  ...
  // 2. ఫ్రంటెండ్ నుండి వచ్చిన డేటాను తీసుకోవడం
  const { teamName, leaderEmail, memberEmails } = data;
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error(`❌ HackerRank API Error (${teamName}):`, resData);
      throw new functions.https.HttpsError(
        "internal", 
    ...
    console.error("⚠️ నెట్‌వర్క్ ఎర్రర్ సంభవించింది:", error);
    throw new functions.https.HttpsError(
      "unknown", 
