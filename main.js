...
function setupResourcesPage() {
  ...
  if (globalPurgeBtn) {
    // 🛠️ FIX: 'style.style.display' డబుల్ ప్రాపర్టీని సరిచేసాను
...
function setupDoubtSolver() {
  ...
  solverForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // 🛠️ FIX: AI సొల్యూషన్ పాయింట్లు పద్ధతిగా కనిపించడానికి 'white-space: pre-wrap' మార్చాము
      solutionDisplay.innerHTML = `
        <div style="background: rgba(16, 185, 129, 0.05); border: 1px solid #10b981; padding: 20px; border-radius: 14px; margin-top: 20px;">
          ...
          <div style="color: #fff; line-height: 1.6; font-size: 14px; white-space: pre-wrap;">
